...
La solucion v2 se compone de 4 workflows de n8n desplegados en un droplet de DigitalOcean: (1) "Investigacion profunda", que recibe solicitudes desde una landing en Vercel via webhook, registra el pedido en Postgres, carga una metodologia editable (playbook) desde la tabla `app_settings` y ejecuta una investigacion en 4 fases con Gemini 2.5 Flash y Google Search grounding (base, profundizacion, sintesis y verificacion adversarial), entregando un articulo HTML con graficos QuickChart y un puntaje de confianza por Gmail y aviso por WhatsApp; (2) "Ideas por WhatsApp", que captura y refina ideas conversando con un AI Agent con memoria por numero de telefono via Evolution API; (3) "Digest semanal", que resume el backlog de ideas pendientes cada lunes; y (4) "Demo sin credenciales", para demostraciones sin depender de APIs externas.

El diferencial del proyecto es la trazabilidad verificable: por reglas duras del playbook, nunca se inventan cifras ni fuentes, todo hecho cita su fuente o se marca `requiere_verificacion`, y solo se grafican cifras que tienen fuente. Ademas, una cuarta fase de **verificacion adversarial** vuelve a auditar el informe con busqueda web y le asigna un puntaje de confianza (0-100) que viaja con cada entregable. El resultado: una investigacion pasa de 4-6 horas a unos 10 minutos de ejecucion mas una revision humana breve, con costo marginal de centavos por investigacion.
...
| Base de datos o Google Sheets | Ambos: Postgres 16 en el droplet (8 tablas `research_*` y `app_settings`, incluida `research_verifications`) y Google Sheets como repositorio de datasets (hoja `datasets`). |
...
3. Integrar Gemini 2.5 Flash con Google Search grounding en un proceso de 4 fases (base, profundizacion, sintesis JSON estricta y verificacion adversarial con puntaje de confianza).
...
Flujo del proceso: una persona envia un tema (y opcionalmente una pregunta) desde la landing o por WhatsApp. Si solo hay tema, el sistema propone 3-5 preguntas investigables y las guarda como ideas. Si hay pregunta, el sistema responde de inmediato "en proceso" y ejecuta la investigacion en 4 fases con busqueda web real (base, profundizacion, sintesis y verificacion adversarial), clasifica afirmaciones (hecho / hipotesis / opinion), extrae datasets con fuente, genera graficos, audita el informe y le asigna un puntaje de confianza, guarda todo en Postgres y Sheets, y entrega el articulo por Gmail con aviso por WhatsApp.
...
INTELIGENCIA
  Gemini 2.5 Flash + Google Search grounding (fases base, profundizacion y verificacion adversarial)
...
| Gemini API (2.5 Flash) | Investigacion en 4 fases (base, profundizacion, sintesis y verificacion adversarial), generacion de preguntas, refinado de ideas, redaccion del digest |
| Google Search grounding | Busqueda web real y citable dentro de Gemini (fases base, profundizacion y verificacion) |
...
Webhook - Nueva investigacion
  ...
  -> IF - Con pregunta
     ...
     [CON pregunta]
       ...
       -> Preparar verificacion -> Gemini - Verificacion (grounding: audita hechos/cifras, puntaje de confianza)
       -> Procesar resultado (valida datasets, arma QuickChart, badge de confianza y articulo HTML)
...
| Investigacion | Preparar verificacion | Arma el prompt de auditoria adversarial sobre la sintesis (hechos, cifras y citas disponibles). |
| Investigacion | Gemini - Verificacion | Fase 4 con grounding: audita hechos y cifras, detecta contradicciones y asigna puntaje de confianza (0-100), nivel y veredicto. |
| Investigacion | Procesar resultado | Integra sintesis y verificacion; valida datasets (solo cifras con fuente), construye URLs QuickChart, badge de confianza, articulo HTML, SQL y filas para Sheets. |
| Investigacion | Postgres - Guardar evidencia | Inserta fuentes, evidencia clasificada, datasets, la verificacion (`research_verifications`) y el articulo como artifact. |
...
2. **Base de datos:** ejecucion de `database_schema_postgres.sql` sobre la base `researchflow` (8 tablas incluida `research_verifications`, playbook inicial en `app_settings` y registro semilla de prueba).
...
1. n8n permite automatizar un proceso intelectual completo: la investigacion en 4 fases con grounding (incluida la verificacion adversarial que puntua la confianza), la clasificacion de evidencia y la entrega multicanal corren sin intervencion humana, y la persona se concentra en revisar y decidir.
...
- Extender la verificacion adversarial ya implementada (Fase 4, mismo modelo con grounding fresco) a un **segundo modelo distinto** como verificador cruzado independiente de los hechos marcados `requiere_verificacion`, reduciendo el sesgo de auto-evaluacion.
...
| Diseno de la solucion | 15 | Arquitectura de 4 workflows, stack en droplet y modelo de datos de 8 tablas (secciones 9 y 10). |
...
| Innovacion y creatividad | 10 | Investigacion en 4 fases con verificacion adversarial y puntaje de confianza, playbook editable en DB, reglas anti-alucinacion y graficos solo con cifras con fuente. |
